#!/usr/bin/env python3
"""
화장품 성분 RAG 챗봇 서버 - 엔터프라이즈급 RAG 파이프라인
...
# LangChain 관련 imports (LangChain 1.0)
from langchain_core.documents import Document
from langchain_text_splitters import RecursiveCharacterTextSplitter
from langchain_community.vectorstores import Chroma
from langchain_community.embeddings import SentenceTransformerEmbeddings
# Memory와 Chains는 LangChain 1.0에서 제거되어 직접 구현
        ...
        if purpose_match:
            ...
            if purposes:
                ...
                if first_purpose_match:
                    ...
                    # good_for 한글화 매핑 (도움이 되는 피부 타입/고민)
                    good_for_map = {
                        "acne": "여드름",
                        "acne-prone": "여드름성",
                        "damaged": "손상된",
...
class EnterpriseRAG:
    ...
    def initialize_components(self):
        ...
        # Mock LLM 초기화 (실제 환경에서는 OpenAI, Anthropic 등 사용)
        self.llm = MockLLM()
    ...
    def create_vectorstore(self):
        ...
        for item in self.ingredients_data:
            # 성분명과 설명을 결합하여 문서 생성 (새로운 파일 구조에 맞게)
        ...
        # ChromaDB 벡터 스토어 생성
        # Chroma 0.4.x 이상에서는 자동으로 저장되므로 persist() 호출 불필요
    ...
    def create_qa_runnable(self):
        ...
        # 커스텀 Runnable 체인 생성 (컨텍스트를 명시적으로 포함)
        ...
        try:
            ...
            for doc in docs:
                ingredient_info = {
                    ...
                    "similarity": 0.8  # ChromaDB에서는 직접 유사도 점수를 제공하지 않으므로 추정값 사용
                }
            ...
            return {
                ...
                "chat_history": memory.messages[-4:] if len(memory.messages) > 4 else memory.messages,  # 최근 2턴의 대화
                "success": True
            }
    ...
    def analyze_product_ingredients(self, ingredients: List[str], skin_type: str) -> Dict:
        ...
        Args:
            ingredients: 성분명 리스트 (예: ["정제수", "프로판다이올", "디메치콘"])
            skin_type: 사용자 피부 타입 (예: "건성", "지성", "복합성")
            
        Returns:
            Dict: {
                "analysis_report": str,  # 종합 분석 리포트
                "good_matches": List[Dict],  # 피부 타입에 맞는 성분
                "bad_matches": List[Dict]  # 피부 타입에 주의가 필요한 성분
            }
        """
        try:
            # 1. 컨텍스트 수집 (Retrieval)
            all_documents = []
            ingredient_info_map = {}  # 성분명 -> 문서 정보 매핑
            
            # 성분명 정규화 함수
            def normalize_ingredient_name(name: str) -> str:
                """성분명 정규화 (공백 제거, 소문자 변환 등)"""
            ...
            for ingredient_name in ingredients:
                ...
                # 1-1. 먼저 정확한 매칭 시도 (ingredients_data에서 직접 찾기)
                ...
                # 1-1-b. 정확 매칭이 없으면 부분 매칭 시도 (OCR 오류 대응)
                if not matched_ingredient and len(normalized_name) >= 3:
                    for item in self.ingredients_data:
                        kor_name = item.get('INGR_KOR_NAME', '')
                        eng_name = item.get('INGR_ENG_NAME', '')
                        
                        # 부분 매칭: 입력된 성분명이 DB의 성분명에 포함되거나, 그 반대
                ...
                # 1-2. 정확한 매칭이 있으면 그것을 사용, 없으면 벡터 검색
            ...
            # 피부 타입 한글화 매핑 (입력된 skin_type이 영어일 경우 대비)
            skin_type_map = {
                "acne": "여드름",
                "acne-prone": "여드름성",
                "damaged": "손상된",
            ...
            for ingredient_name, info in ingredient_info_map.items():
                ...
                if bad_for:
                    ...
                    bad_for_list_normalized = [
                        bad_for_map_local.get(item.lower(), item) 
                    ...
                    # 일반적인 주의사항이 있는 경우 (피부 타입과 무관)
                    # 영어 키워드와 한글 키워드 모두 확인
...
1. [주요 성분 목적]을 보고 이 제품의 핵심 목적과 제품 타입을 추론하여 "이 화장품은(는) [제품 타입/목적]으로 보입니다." 또는 "이 화장품은(는) [핵심 목적]에 중점을 둔 제품으로 보입니다."로 문장을 시작하세요.
   - 예: "향수" 제품인 경우 "향수 제품으로 보입니다" 또는 "향료 성분에 중점을 둔 제품으로 보입니다"
   - 예: "보습" 제품인 경우 "보습에 중점을 둔 제품으로 보입니다"

2. [좋은 성분 목록]과 [좋은 성분 세부정보]가 있으면, "이 제품에는 [성분명 1], [성분명 2] 등이 포함되어 있어 [주요 효능]에 도움을 줄 수 있습니다." 형식으로 긍정적인 부분을 요약하세요.
   - 피부 타입별 정보가 있으면 언급하되, 없으면 일반적인 효능을 설명하세요.

3. [주의 성분 목록]과 [주의 성분 세부정보]가 있으면, "다만, [성분명 1] 등은 [주의 이유]을 유발할 수 있으니 [적용 대상] 참고하시기 바랍니다." 형식으로 주의점을 요약하세요.
   - 민감성 피부에 대한 주의사항이 있으면 반드시 언급하세요.
   - 피부 타입과 무관한 일반적인 주의사항도 포함하세요.

4. 제품 타입에 따라 적절한 설명을 제공하세요:
   - 향수 제품: 피부 타입보다는 향료 성분과 일반적인 주의사항(알레르기, 민감성 등)에 중점을 둡니다.
   - 스킨케어 제품: 피부 타입별 정보와 일반적인 주의사항을 모두 포함합니다.

5. 긍정/주의 성분이 없다면, 해당 부분을 자연스럽게 생략하거나 "특별히 긍정적인/주의가 필요한 성분은 보이지 않습니다."라고 언급하세요.

6. 절대 "이 제품은 [피부타입]입니다." 처럼 어색한 문장을 반복하지 마세요.
...
# Flask 앱 초기화
app = Flask(__name__)
CORS(app)  # Android 앱에서 접근 가능하도록 CORS 설정

# 엔터프라이즈급 RAG 시스템 초기화
print("🚀 엔터프라이즈급 RAG 시스템 초기화 시작...")
# ingredients.json 파일 경로 설정 (프로젝트 루트 기준)
script_dir = os.path.dirname(os.path.abspath(__file__))
project_root = os.path.dirname(script_dir)  # backend/의 상위 디렉토리
...
def health_check():
    ...
    return jsonify({
        'status': 'healthy',
        'message': '엔터프라이즈급 RAG 서버가 정상 작동 중입니다',
...
def search_ingredients():
    ...
    try:
        ...
        session_id = data.get('session_id', None)  # 선택적 세션 ID
        
        if not query:
            return jsonify({'error': '검색어를 입력해주세요'}), 400
        
        # 엔터프라이즈급 RAG 시스템으로 검색 및 답변 생성
        result = rag_system.search_ingredients(query, session_id)
...
def clear_chat_history():
    ...
    try:
        ...
        result = rag_system.clear_chat_history(session_id)
        return jsonify(result)
    ...
    try:
        ...
        for session_id, memory in rag_system.chat_sessions.items():
            sessions.append({
                ...
                'last_activity': datetime.now().isoformat()  # 실제로는 마지막 활동 시간을 추적해야 함
            })
...
@app.route('/rag/info', methods=['GET'])
def get_rag_info():
    """RAG 시스템 정보 조회"""
    try:
        return jsonify({
...
def analyze_product():
    ...
    Request Body:
        {
            "ingredients": ["정제수", "프로판다이올", "디메치콘", ...],
            "skin_type": "건성"
        }
    
    Response Body:
        {
            "analysis_report": "종합 분석 리포트...",
            "good_matches": [
                {"name": "글리세린", "purpose": "보습제"},
                ...
            ],
            "bad_matches": [
                {"name": "에탄올", "description": "피부 건조 유발 가능"},
                ...
            ],
            "success": true
        }
    ...
    try:
        ...
        if not ingredients or not isinstance(ingredients, list):
            return jsonify({
                'error': 'ingredients 필드는 성분명 리스트여야 합니다.',
                'success': False
            }), 400
        
        if not skin_type or not isinstance(skin_type, str):
            return jsonify({
                'error': 'skin_type 필드는 문자열이어야 합니다.',
                'success': False
            }), 400
        
        # RAG 시스템으로 제품 분석 수행
        result = rag_system.analyze_product_ingredients(ingredients, skin_type)
...
if __name__ == '__main__':
    print("🚀 엔터프라이즈급 화장품 성분 RAG 챗봇 서버 시작")
    ...
    print("  - POST /analyze_product - 제품 성분 종합 분석 (새로운 기능)")
