...
(✨수정) 나아가, 특정 성분에 대해 궁금한 점이 생기면 **Chat History(대화 기록)**를 기반으로 후속 질문이 가능한 전문 RAG 시스템을 통해, 단순 정보 조회를 넘어선 전문가 수준의 대화형 설명을 제공한다.
...
핵심 가치 전달: "나에게 맞는 화장품을 쉽고 빠르게 찾고, 궁금한 점은 AI 전문가에게 바로 질문한다"는 앱의 핵심 가치를 명확한 UI/UX를 통해 전달한다.
...
(✨신규) 후속 질문 비율: AI 상세 설명을 조회한 사용자 중, 후속 질문(Chat History)을 1회 이상 수행하는 비율 (20% 이상 목표)
...
(✨수정) As a 성분에 관심이 많은 학생,

I want to '시어버터' 성분이 왜 지성 피부에겐 주의해야 하는지 AI 설명을 읽어본 뒤, "그럼 지성 피부가 대신 쓸만한 보습 성분은 뭐야?"라고 후속 질문을 하고 싶다.

So that 대화의 맥락(Chat History)을 기억하는 AI를 통해 전문 지식을 연속적으로 얻고 싶다.
...
(✨수정) REQ-03-D (RAG 백엔드 연동): 상세 정보 화면 진입 시, 앱은 해당 성분명(예: '시어버터')을 초기 질문으로 하여 Python RAG 백엔드 API (REQ-05-B)를 호출해야 한다.

(✨수정) REQ-03-E (AI 설명 표시): RAG 백엔드로부터 전달받은 AI 생성 설명(JSON 응답)을 파싱하여 상세 정보 화면의 'AI 전문가 설명' 영역에 표시해야 한다.

(✨신규) REQ-03-F (후속 질문 UI): 'AI 전문가 설명' 하단에는 사용자가 추가 질문을 입력하고 전송할 수 있는 채팅 입력창(Text Field)과 전송 버튼이 제공되어야 한다.

(✨신규) REQ-03-G (세션 관리): 사용자가 후속 질문을 전송할 때, 앱은 RAG 백엔드와의 대화 맥락(Chat History)을 유지하기 위해 API 호출 시 session_id를 함께 전달해야 한다. (최초 질문 시 백엔드에서 session_id 발급)
...
REQ-05-B (API 엔드포인트): 최소 1개 이상의 대화형 엔드포인트(예: POST /api/v1/chat)를 제공해야 한다.

Request Body: { "query": "string", "session_id": "string (optional)" }

Response Body: { "answer": "string", "session_id": "string", "source_docs": [] }
...
Contextualize (맥락화): session_id를 기반으로 Chat History (대화 기록)를 조회하여, 사용자의 현재 질문을 이전 대화의 맥락을 포함하도록 재구성한다. (예: RunnableWithMessageHistory 활용)
...
History Update (기록): 사용자의 질문과 LLM의 답변을 해당 session_id의 Chat History에 저장한다.
...
graph TD
    ...
    H --> I[표시: 기본정보 + AI 설명<br>(RAG 백엔드 1차 호출)];
    I --> J[표시: 후속 질문 입력 UI];
    J -- 사용자: 후속 질문 입력/전송 --> K((RAG 백엔드 N차 호출<br>/w Chat History));
    K --> I;
...
(✨신규) RAG 백엔드의 AI 상세 설명 API (REQ-03-D) 최초 응답 시간은 5초 이내, 후속 질문 응답 시간은 3초 이내를 목표로 한다.

사용성 (Usability): 별도의 가이드 없이도 사용자가 직관적으로 스캔하고, 결과를 확인하며, 후속 질문을 할 수 있는 UI/UX를 제공해야 한다.
...
온라인 기능: REQ-03의 AI 상세 설명 및 후속 질문 기능은 RAG 백엔드 서버와의 네트워크 통신을 필수로 요구한다.
...
(✨수정) 위에 명시된 REQ-01 ~ REQ-05의 모든 기능 포함을 목표로 한다.

즉, 사용자가 성분 스캔, 결과 요약 확인, 특정 성분 상세 정보 확인, RAG/Chat History 기반 후속 질문까지의 핵심 플로우 전체를 경험할 수 있는 버전을 릴리즈한다.

8. 범위 외 (Out of Scope)
사용자 계정 및 프로필: v1.0에서는 회원가입, 로그인, 개인 피부 타입 설정 기능을 포함하지 않는다. (분석 요약은 '일반적인' 피부 타입을 기준으로 함)

스캔 히스토리: 과거에 분석했던 제품 기록을 앱에 저장하는 기능은 포함하지 않는다. (단, RAG 백엔드는 대화 세션(session_id) 기록만 관리함)
